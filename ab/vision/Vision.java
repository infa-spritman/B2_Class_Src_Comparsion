...
public class Vision {
	...
	//find Terrain using boofcv
	public List<Point> findTerrain(List<Rectangle> pigs, List<Rectangle> wood, List<Rectangle> ice, List<Rectangle> stones) {
		BufferedImage image = this.scrShot;
		...
		canny.process(gray,0.1f,0.9f,edgeImage);
		...
		List<Point> terrain = new ArrayList<Point>(0);
		for(Contour _contour : contours) {
			for(Point2D_I32 _point2d : _contour.external) {
				Point temp = new Point(_point2d.getX(),_point2d.getY());
				terrain.add(temp);
			}
		}

		//System.out.println("Number of points in terrain: " + terrain.size());
		ArrayList<Point> finTerrain = new ArrayList<Point>(0);
		finTerrain.addAll(terrain);
		ArrayList<Rectangle> allRects= new ArrayList<Rectangle>(0);
		allRects.addAll(pigs);
		allRects.addAll(wood);
		allRects.addAll(ice);
		allRects.addAll(stones);
		for(Point p : terrain) {
